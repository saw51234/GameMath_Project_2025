...
public class Dijkstra : MonoBehaviour
{
    ...
    void Start()
    {
        var path = Dijkstras(map, start, goal);
    ...
    List<Vector2Int> Dijkstras(int[,] map, Vector2Int start, Vector2Int goal)
    {
        ...
        while (open.Count > 0)
        {
            ...
            foreach (var d in dirs)
            {
                ...
                if (!InBounds(map, nx, ny)) continue;      // 맵 범위 체크
                if (map[nx, ny] == 0) continue;            // 벽 체크

                int moveCost = TileCost(map[nx, ny]);      // (nx, ny) 이동 비용
                if (moveCost == int.MaxValue) continue;    // 이동 불가 타일
